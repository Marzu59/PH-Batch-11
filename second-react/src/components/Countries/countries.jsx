...
const Countries = ({countriespromis}) => {
    ...
    const [vistedFlags, setVisitedFlags] = useState([])
      function handlevisitdFlag(flag){
    ...
    return (
        <div >
            ...
            <ol>
              {
                visitedCountries.map(contyy=> <li>{contyy.name.common}</li>)
              }
              <div className='visited-flags-container'>
                { 
                  vistedFlags.map(visitedflag=> <img src={visitedflag} alt="" /> )
            ...
            <div className='countries'>
            {
                countries.map(country=> <Country key={country.cca3} 
                    handleVisitedCountries={handleVisitedCountries}  country={country} handlevisitdFlag={handlevisitdFlag} ></Country>  )
            }
            </div>
